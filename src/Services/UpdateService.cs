...
/// <summary>
/// Checks GitHub releases for updates (Jon2G/OneTap-Habits).
/// </summary>
[Microsoft.Maui.Controls.Internals.Preserve(AllMembers = true)]
public class UpdateService
{
	...
	private const string RepoName = "OneTap-Habits";
	private static readonly string ApiUrl = $"https://api.github.com/repos/{RepoOwner}/{RepoName}/releases/latest";

	static UpdateService()
	{
		HttpClient.DefaultRequestHeaders.Add("User-Agent", "OneTapHabits/1.0");
	...
	public async Task<UpdateInfo?> CheckForUpdatesAsync()
	{
		try
		{
			...
			var remoteVersionStr = release.TagName.StartsWith("v", StringComparison.OrdinalIgnoreCase)
				? release.TagName[1..]
				: release.TagName;
			...
			var apkAsset = release.Assets.FirstOrDefault(a =>
				                    a.Name.StartsWith("OneTapHabits-", StringComparison.OrdinalIgnoreCase) &&
				                    a.Name.EndsWith(".apk", StringComparison.OrdinalIgnoreCase))
			                ?? release.Assets.FirstOrDefault(a =>
				                a.Name.EndsWith(".apk", StringComparison.OrdinalIgnoreCase));

			var hasPlatformRelease = apkAsset != null;
		...
		CancellationToken cancellationToken = default)
	{
		try
		{
			var fileName = $"OneTapHabits-{version}.apk";
			var destPath = Path.Combine(FileSystem.CacheDirectory, fileName);

			using var downloadClient = new HttpClient();
			downloadClient.DefaultRequestHeaders.Add("User-Agent", "OneTapHabits/1.0");
	...
	public async Task<string?> GetChangelogForVersionAsync(string version)
	{
		try
		{
			...
			request.Headers.Add("User-Agent", "OneTapHabits/1.0");
